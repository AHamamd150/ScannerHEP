...
##########################
##########################
def train_refine(AI_X,AI_Y):
    
    while  AI_Y.sum() < npoints:
        x_init1 = AI_X[AI_Y==1][:AI_Y.sum()]; y_init1 = AI_Y[AI_Y==1][:AI_Y.sum()];
        x_init0 = AI_X[AI_Y==0][:AI_Y.sum()]; y_init0 = AI_Y[AI_Y==0][:AI_Y.sum()];
        X_vegas = np.concatenate((x_init1,x_init0),axis=0)
        Y_vegas = np.concatenate((y_init1,y_init0),axis=0)
        
        x_test = generate_init_HEP(batch_size,TotVarScanned,paths,Lesh,VarLabel,VarMin,VarMax)
        x_test_vegas = x_test[: round((1-random_frac)*batch_size)]
        x_test_rand = x_test[round((1-random_frac)*batch_size):]
        limits = np.column_stack((VarMin,VarMax))
        Veg_map = vegas_map_samples(X_vegas,Y_vegas,limits)
        x,_ = Veg_map(x_test)
        x_refined = np.concatenate((x,x_test_rand),axis=0) 
        for xx in range(0,x_refined.shape[0]):
            if AI_Y.sum() >=  npoints: break
            print ("""-------------------------------------------
NUMBER OF COLLECTED POINTS %s = \t %i of  (%i)
-------------------------------------------"""%(str(pointN[-1]),(AI_Y.sum()+1),npoints))
            newrunfile = open('newfile','w')
            oldrunfile = open(str(Lesh),'r+')
            AI_L = []
            for line in oldrunfile: 
                    ...
                    if (TotConstScanned !=0):
                       ...
                       if (MadGraph != 0):
                          if os.path.exists(str(paths)+'spc.slha'):
                              ...
                              madevent_run(str(paths)+'spc.slha')                              
                       if os.path.exists(str(paths)+'spc.slha'):      
                          os.rename('spc.slha','SPheno.spc.%s_%i'%(str(SPHENOMODEL),(AI_Y.sum()+1)))
                          shutil.move('SPheno.spc.%s_%i'%(str(SPHENOMODEL),(AI_Y.sum()+1)),path+"/Spec_out_%s/"%(str(SPHENOMODEL)))
                          os.rename(str(Lesh),str(Lesh)+'%s_%i'%(str(SPHENOMODEL[-1]),(AI_Y.sum()+1)))
                          shutil.copy(str(Lesh)+'%s_%i'%(str(SPHENOMODEL[-1]),(AI_Y.sum()+1)),path+"/Les_out_%s"%(str(SPHENOMODEL)))
                          os.rename(str(Lesh)+'%s_%i'%(str(SPHENOMODEL[-1]),(AI_Y.sum()+1)),str(Lesh))
...
##########################################################
##########################################################
x_init, y_init = train_init(points_init)
#x_init_1 = x_init[y_init==1][:points_init]; y_init_1 = y_init[y_init==1][:points_init];
#x_init_0 = x_init[y_init==0][:points_init]; y_init_0 = y_init[y_init==0][:points_init];
#x_refine = np.concatenate((x_init_1,x_init_0),axis=0)
#y_refine = np.concatenate((y_init_1,y_init_0),axis=0)

train_refine(x_init,y_init)
################################
########################
